...
/** A banner larger than this is not worth decoding for OCR. */
const MAX_IMAGE_BYTES = 8_000_000

/** Bounds a hung offscreen OCR request. */
const RECOGNIZE_TIMEOUT_MS = 30_000

/** Total OCR time one message may consume, across all of its images. */
...
/** Tesseract needs Web Workers, so the service worker creates this document on demand. */
async function ensureOffscreenDocument(): Promise<void> {
  const existing = await chrome.runtime.getContexts(offscreenFilter())
  if (existing.length > 0) return

  // Chrome permits one offscreen document, so concurrent callers share creation.
...
/** Releases the Tesseract worker and language data when a sync slice finishes. */
export async function closeOcrEngine(): Promise<void> {
  // Do not leave a document behind if creation is still in flight.
...
function toDataUrl(bytes: ArrayBuffer, contentType: string): string {
  ...
  let binary = ''
  // Chunking avoids exceeding the argument limit on large images.
...
/** Remote fetches omit credentials and referrers, though the request itself is still visible. */
...
async function loadImage(
  ...
  // Gmail either embeds a small part directly or exposes an attachment ID.
...
/** Reads candidates until the time budget expires, retaining only trustworthy OCR text. */
